...
export default function SocialCards({ cards, onActiveChange, onCardClick }: SocialCardsProps) {
  ...
  return (
    <section className="flex flex-col items-center w-full relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] aspect-[2/1] md:aspect-[3/1]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-48 h-64 md:w-72 md:h-[26rem] rounded-xl overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.08)] border border-black/5 bg-white">
            ...
            return (
              <div
                ...
                onClick={() => {
                  ...
                  // Optionally, also cycle to make this card the center one
                  if (needsPagination && !isAnimating.current) {
                    const diff = (index - centerIndex + totalCards) % totalCards;
                    if (diff !== 0) {
                      const dir = diff <= totalCards / 2 ? "right" : "left";
                      // Simple implementation: just cycle once in the correct direction
                      cycle(dir);
                    }
                  }
                }}
